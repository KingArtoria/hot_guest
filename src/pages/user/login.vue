<template>
  <view>
    <view class="content">
      <!-- 关闭按钮 -->
      <image class="content_1" src="http://39.106.208.234/pic/img_/close_dl.png" />
      <!-- 微信登录 -->
      ...
      <view class="content_4" v-show="currentLogin.type != 1">
        <!-- 标题 -->
        <view class="content_4_1" v-show="currentLogin.type == 2">手机号码登录</view>
        <view class="content_4_1" v-show="currentLogin.type == 3">密码登录</view>
        <view class="content_4_1" v-show="currentLogin.type == 4">注册账号</view>
        <!-- 表单 -->
...
import { getVerificationCode, passwordLogin, phoneLogin, register } from '../../utils/api';
export default {
  data() {
    return {
      // 登录选项
      loginOptions: [
        { type: 2, name: '手机登录', icon: 'http://39.106.208.234/pic/img_/yanzhengma.png' },
        { type: 3, name: '密码登录', icon: 'http://39.106.208.234/pic/img_/mima_.png' },
        { type: 4, name: '注册账号', icon: 'http://39.106.208.234/pic/img_/zhuce.png' },
      ],
      // 当前登录方式
      currentLogin: {
        type: 1,
